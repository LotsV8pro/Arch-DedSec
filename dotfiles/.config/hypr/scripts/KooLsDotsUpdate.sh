...
if [ -z "$github_version" ]; then
  notify-send -i "$iDIR/error.png" 'ctOS:' "Unable to determine GitHub version."
  exit 1
fi

# Comparing local and github versions
if [ "$(echo -e "$github_version\n$local_version" | sort -V | head -n 1)" = "$github_version" ]; then
   notify-send -i "$iDIR/note.png" "ctOS:" "No update available"
  exit 0
else
  # update available
  ...
  response=$($notify_cmd_shot "ctOS:" "Update available! Update now?")
